...
export async function GET(request:Request) { return api(async () => {
  ...
  return getProfile(user.id, new URL(request.url).searchParams.get("brand") || undefined);
}); }
export async function PUT(request: Request) { return api(async () => {
  ...
  return saveProfile(user.id, body as BrandProfile, stringField(body,"brandId",100,false) || undefined);
}); }
